...
from model3 import CnnActorCriticNetwork3, RNDModel3
from envs import *
...
def compute_intrinsic_reward(rnd, device, next_obs):
    ...
    initial_next_feature = rnd.initial(next_obs)
    intrinsic_reward = (target_next_feature - predict_next_feature).pow(2).sum(1) / 2 / ((initial_next_feature - target_next_feature).pow(2).sum(1))*100
...
def train_model(args, device, output_size, model, rnd, optimizer, s_batch, target_ext_batch, target_int_batch, y_batch, adv_batch, next_obs_batch, old_action_probs):
    ...
    for i in range(epoch):
        ...
        for j in range(int(len(s_batch) / args.batch_size)):
            ...
            predict_next_state_feature, target_next_state_feature, initial_next_state_feature = rnd(next_obs_batch[sample_idx])
...
def main():
    ...
    initial_path = os.path.join(args.save_dir, args.env_name + '.initial')
    ...
    model = CnnActorCriticNetwork3(input_size, output_size, args.use_noisy_net)
    rnd = RNDModel3(input_size, output_size)
    ...
    if args.load_model:
        if args.cuda:
            ...
            rnd.initial.load_state_dict(toch.load(initial_path))
            ...
            rnd.initial.load_state_dict(toch.load(initial_path, map_location='cpu'))
    ...
    while True:
        ...
        # None Episodic
        int_target, int_adv = make_train_data(total_int_reward*(1-0.5*total_done),
        ...
        if global_step % (args.num_worker * args.num_step * args.save_interval) == 0:
            ...
            torch.save(rnd.initial.state_dict(), initial_path)
